        ...
        <div class="container">
                    ...
                    <form method="get" class="row">
                        <div class="form-group col-6 mt-2">
                            ...
                            <select id="type" class="form-control" name="type">
                                <option {{ request()->type == 'etudiant' ? 'selected' : '' }} value="etudiant">Etudiant
                                </option>
                                <option {{ request()->type == 'insertion' ? 'selected' : '' }} value="insertion">Taux
                                    d'insertion</option>
                                <option {{ request()->type == 'contractuel' ? 'selected' : '' }} value="contractuel">
                                    Personnel contractuels</option>
                                <option {{ request()->type == 'non_contractuel' ? 'selected' : '' }}
                                    value="non_contractuel">Personnel titulaires</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-6 mt-2">
                            ...
                            <select class="form-control" name="trie">
                                ...
                                <option {{ request()->trie == 'desc' ? 'selected' : '' }} value="desc">Décroissant
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="form-group col-6 mt-2">
                            ...
                            <select class="form-control" name="nbr_result">
                                @for ($i = 0; $i < 50; $i++)
                                    <option {{ request()->nbr_result == $i + 1 ? 'selected' : '' }}
            ...
            <div class="card mt-5 shadow" style="border: none">
                <div class="card-header">
                    ...
                    <i>Sélectionner la liste des établissements que vous souhaitez comparer, puis lancer la comparaison.</i>
                </div>
                <div class="card-body">
                    <form method="get" class="row">
                        <div class="form-group">
                            <label for="type" class="pb-1">Comparaison</label>
                            <select id="type" class="js-example-basic-multiple form-control" name="comparaison[]"
                                multiple="multiple">
                                    ...
                                    @if (request()->type == 'insertion')
                                        @foreach ($universites as $universite)
                                            <tr>
                                                ...
                                                <td> {{ $universite->universite->inscrits_ . (request()->annee ?? '0') }}
                                                </td>
                                                <td> {{ $universite->domaine }} </td>
                                                <td> {{ $universite->discipline }} </td>
                                                <td> {{ $universite->taux_dinsertion }} </td>
                                                <td> {{ $universite->universite->personnels->where('type_personnel', 'contractuels')->count() }}
                                                </td>
                                                <td> {{ $universite->universite->personnels->where('type_personnel', '!=', 'contractuels')->count() }}
                                                </td>
                                                <td class="text-center">
                                                    <a href="{{ route('universites.show', $universite->id) }}"><i
                                    ...
                                    @else
                                        @foreach ($universites as $universite)
                                            <tr>
                                                ...
                                                <td> {{ $universite->personnels->where('type_personnel', 'contractuels')->count() }}
                                                </td>
                                                <td> {{ $universite->personnels->where('type_personnel', '!=', 'contractuels')->count() }}
                                                </td>
                                                <td class="text-center">
